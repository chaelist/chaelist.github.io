...
4. index(): 해당 element의 index number를 가져옴
```python
# index
x = [1, 2, 3, 4]
...
c
```
3. sum(): list에 있는 모든 값을 다 더해줌 (*값이 모두 integer 혹은 float여야 함)
```python
# sum
x = [1, 2, 3, 4]
sum(x)
...
5. del x[index]: 해당 index number를 갖는 element를 삭제
```python
# del x[index]
x = [1, 2, 3, 4]
...
6. x.sort():list 안의 값들을 순서대로 정렬해준다 (숫자는 오름차순, string은 첫 글자 알파벳순)
```python
# x.sort()
x = [1, 4, 3, 2, 5]
x.sort()
